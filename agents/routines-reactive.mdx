...
Reactive routines fire in response to events rather than a schedule. Use them to automate workflows that should happen when something occurs — a file is created, a webhook fires, or an internal agent event is raised.
...
### New File Triggers Summary

When any file is written to the `inbox/` memory path, summarize it and write a processed version:
...
```json
{
  ...
  "trigger": {
    "type": "event",
    "event": "memory.write",
    "filter": { "path_prefix": "inbox/" }
  },
  "action": {
    "prompt": "A new document was written to inbox/. Read it, extract the key points, and write a structured summary to processed/. Delete the original from inbox/ after processing."
  },
  "guardrails": {
    "max_tool_calls": 10,
    "timeout_secs": 60,
    "rate_limit": { "max_runs": 50, "window_secs": 3600 }
  }
}
```

### Webhook Triggers Deploy Notification

When CI/CD signals a successful deploy, log the event and check for any related open issues:
...
```json
{
  ...
  "action": {
    "prompt": "A deployment just completed. Parse the webhook body for environment, version, and commit. Write a deploy record to memory at deployments/<date>.md. Search memory for any open issues related to this version and flag them."
  },
  "guardrails": {
    "max_tool_calls": 15,
    "timeout_secs": 120,
    "rate_limit": { "max_runs": 20, "window_secs": 3600 }
  }
}
```

### Failed Job Escalation

When any job fails, write an incident record:

```json
{
  "name": "job-failure-escalation",
  "trigger": {
    "type": "event",
    "event": "job.failed"
  },
  "action": {
    "prompt": "A job just failed. Get the job details using job_status. Write an incident record to memory at incidents/<date>-<job-id>.md with the job description, error, and timestamp."
  },
  "guardrails": {
    "max_tool_calls": 5,
    "timeout_secs": 60,
...
<CardGroup cols={2}>
  <Card title="Cron Routines" icon="clock" href="/agents/routines-cron">
    Schedule recurring tasks on a time-based schedule
  </Card>

  <Card title="Routines Overview" icon="settings" href="/agents/routines">
