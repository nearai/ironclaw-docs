...
All four files live in the workspace root and are written using `memory_write`.
...
Use `memory_write` to create or update any identity file:

```
Write AGENTS.md with the following content:
# Agent Instructions
Be concise. Search memory before answering questions about past work.
```

Or write them directly via the memory tools in conversation:

```
memory_write path="USER.md" content="..."
```

## HEARTBEAT.md — A Special File

`HEARTBEAT.md` is not injected into the system prompt, but it serves a similar purpose for the heartbeat system. The agent reads it every 30 minutes and executes its checklist as a proactive job.

See [Heartbeat](/agents/heartbeat) for details on the HEARTBEAT.md format.

## Per-Workspace Identity

Identity files are scoped to the workspace. Different workspaces can have different AGENTS.md, SOUL.md, USER.md, and IDENTITY.md files — allowing you to give the agent different personas and instructions depending on the project context.
...
<CardGroup cols={2}>
  ...
  <Card title="Heartbeat" icon="activity" href="/agents/heartbeat">
    HEARTBEAT.md and proactive periodic execution
  </Card>
</CardGroup>
