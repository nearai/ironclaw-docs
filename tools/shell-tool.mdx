...
<Warning>
The shell tool requires `ALLOW_LOCAL_TOOLS=true` and gives the agent direct command execution on your machine. Enable only when you trust the agent to operate at this level. Consider using the Docker sandbox instead for untrusted workloads.
</Warning>
...
```json
{
  ...
  "properties": {
    ...
    "timeout_secs": {
      "type": "integer",
      "description": "Maximum time to wait for the command to complete. Default: 30.",
      "default": 30
    }
  },
  "required": ["command"]
...
<CardGroup cols={3}>
  <Card title="File Tools" icon="file" href="/tools/file-tools">
    Filesystem access without command execution
  </Card>

  <Card title="Sandbox" icon="container" href="/security/sandbox">
    Docker-based isolation for untrusted execution
  </Card>

  <Card title="Safety Layer" icon="shield" href="/security/safety-layer">
    Sanitizer, policy, and leak detector
  </Card>
</CardGroup>
