...
![密钥对创建对话框](/images/infrastructure/amazon/amazon-landing.png)
...
![EC2 实例配置](/images/infrastructure/amazon/amazon-keypair.png)
...
[安全指南](/zh/infrastructure/security) 同样适用于您的 EC2 实例，但有一点不同：由于您已以 `ubuntu`（非 root 的 sudo 用户）身份登录，可以 **跳过"创建新用户"步骤** ——`ubuntu` 已承担了该角色。
